      ...
      <div className="glass-panel rounded-3xl max-w-xl w-full p-6 sm:p-7 relative border border-slate-200/70 dark:border-slate-800/80 shadow-2xl">
        ...
        <form onSubmit={handleAssign} className="space-y-4 my-3">
          ...
          <div>
            ...
            {loading ? (
              <div className="text-center py-6 text-xs text-slate-400">Loading workers...</div>
            ) : (
                  ...
                  return (
                    ...
                    >
                          ...
                          {w.specialties?.length > 0 && (
                            <div className="flex gap-1 mt-1 flex-wrap">
                              {w.specialties.map((s, idx) => (
                                <span
                                  key={idx}
                                  className="text-[9px] px-1.5 py-0.2 rounded bg-slate-200/70 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300"
                                >
                                  {s}
                                </span>
                              ))}
                      ...
                      <div className="text-right shrink-0">
                        <span className="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-200/60 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400">
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-200/50 dark:border-slate-800/60">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-indigo-600 to-cyan-500 hover:opacity-95 shadow-glowBrand flex items-center gap-1.5 cursor-pointer"
            >

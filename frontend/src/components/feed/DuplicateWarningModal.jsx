  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="glass-panel rounded-3xl max-w-xl w-full p-6 sm:p-7 relative border border-indigo-500/40 shadow-2xl">
        ...
        <div className="space-y-3 max-h-72 overflow-y-auto pr-1 my-4">
          {duplicates.map((item, idx) => {
            ...
            return (
              <div
                key={issue._id || idx}
                className="p-4 rounded-2xl glass-card border border-slate-200/70 dark:border-slate-800/80 hover:border-indigo-500/40 transition-all"
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <StatusBadge status={issue.status} />
                    <PriorityBadge priority={issue.priority} />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-600 dark:text-cyan-400 border border-indigo-500/30">
                ...
                <div className="flex items-center justify-between gap-2 mt-3 pt-2.5 border-t border-slate-200/50 dark:border-slate-800/60">
                  <div className="flex gap-1 flex-wrap">
                    {item.reasons.map((r, i) => (
                      <span
                        key={i}
                        className="text-[9px] px-2 py-0.5 rounded-md bg-slate-200/60 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400"
                      >
                        {r}
                      </span>
                    ))}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-indigo-600 to-cyan-500 shadow-glowBrand hover:opacity-95 transition-all cursor-pointer"
                  >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-slate-200/50 dark:border-slate-800/60">
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-semibold glass-card text-slate-600 dark:text-slate-300 hover:text-slate-900 cursor-pointer"
          >
            Review My Complaint
          </button>
          <button
            onClick={onProceedAnyway}
            className="w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-200/70 dark:bg-slate-800/70 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
